...
const PreviewArea = () => {
    ...
    const newSlide = () => {
        ...
        dispatch(changeSlide({index, contents}))
        // allSlide中新建
        dispatch(addSlide());
        // 切换新幻灯片至编辑区
    ...
    // 切换编辑的幻灯片
    const changeEditingSlide = (index: number) => {
        // 保存editingSlide至allSlide 
        dispatch(changeSlide({index, contents}))
        // 切换新幻灯片至编辑区
        dispatch(setEditingSlide({index,contents:slides[index].contents}));
    }
    ...
    return (
        ...
        <div className={style.preview}>
            {slides.map((ele,index)=>{
                return <div key={index} style={{width:'100px',height:'100px', border: '1px solid #5851b4'}} onClick={() => changeEditingSlide(index)}></div>;
            })}
        </div>
        </div>
    );
